...
check(index.includes("./src/features/real-platform-v660.js?v=10.3.0"), "Real platform module loaded through V10.3 cache key");
...
check(readFileSync(resolve(root, "service-worker.js"), "utf8").includes('win11-simulator-v10.3.0'), "PWA cache bumped to V10.3.0");
...
check(readFileSync(resolve(root, "src/features/real-platform-v660.js"), "utf8").includes('service-worker.js?v=10.3.0'), "PWA registration references V10.3 Service Worker");
...
const windowManagerV103=readFileSync(resolve(root,"src/core/window-manager-v103.js"),"utf8");
const bootV103=readFileSync(resolve(root,"src/core/boot.js"),"utf8");
const packageV103=JSON.parse(readFileSync(resolve(root,"package.json"),"utf8"));
const serviceWorkerV103=readFileSync(resolve(root,"service-worker.js"),"utf8");
const browserAuditV103=readFileSync(resolve(root,"tools/browser_audit.mjs"),"utf8");
...
check(index.includes("./src/core/window-manager-v103.js?v=10.3.0"),"Window Manager V10.3 module loaded");
check(index.indexOf("boot-recovery-v101.js?v=10.1.0")<index.indexOf("desktop-taskbar-v102.js?v=10.2.0")&&index.indexOf("desktop-taskbar-v102.js?v=10.2.0")<index.indexOf("window-manager-v103.js?v=10.3.0")&&index.indexOf("window-manager-v103.js?v=10.3.0")<index.indexOf("core/boot.js?v=10.3.0"),"V10 core load order is incremental and safe");
check(windowManagerV103.includes('VERSION="10.3.0"')&&windowManagerV103.includes("globalThis.Win11WindowManagerV10"),"Window Manager V10.3 bridge present");
check(windowManagerV103.includes("repairIdentityAndDesktop")&&windowManagerV103.includes("repairFocus")&&windowManagerV103.includes("repairSnap"),"Window Manager V10.3 self-reconcile present");
check(windowManagerV103.includes("window-lifecycle-contract")&&windowManagerV103.includes("virtual-desktop-contract"),"Window Manager V10.3 capability markers present");
check(bootV103.includes('VERSION="10.3.0"')&&bootV103.includes('platform.start("window-manager-v10")'),"Boot V10.3 starts native Window Manager");
check(bootV103.includes("windowManager:globalThis.Win11WindowManagerV10"),"Simulator diagnostics include Window Manager V10.3");
check(index.includes("./src/core/boot.js?v=10.3.0"),"Boot V10.3 module loaded");
check(packageV103.version==="10.3.0","Package version advanced to V10.3.0");
check(packageV103.scripts?.test?.includes("window-manager-v103.mjs"),"Window Manager V10.3 unit tests wired into npm test");
check(serviceWorkerV103.includes('win11-simulator-v10.3.0'),"Service Worker cache is V10.3.0");
check(serviceWorkerV103.includes("./src/core/window-manager-v103.js?v=10.3.0"),"Window Manager V10.3 module precached");
check(browserAuditV103.includes("Window Manager V10.3")&&browserAuditV103.includes('win11-simulator-v10.3.0'),"Browser audit covers V10.3 Window Manager and cache");
const readmeV103=readFileSync(resolve(root,"README.md"),"utf8");
check(readmeV103.includes("**V10.3 — Window Manager**"),"README advertises V10.3 Window Manager");
check(readmeV103.length<20000&&readmeV103.includes("[docs/CHANGELOG.md](docs/CHANGELOG.md)"),"README is concise and links archived version history");
check(existsSync(resolve(root,"docs/CHANGELOG.md")),"Detailed version history archived outside README");
