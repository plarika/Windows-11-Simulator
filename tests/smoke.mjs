...
check(index.includes("./src/features/real-platform-v660.js?v=10.2.0"), "Real platform module loaded through V10.2 cache key");
...
check(bootSource.includes("globalThis.Win11SessionManager?.handleBootComplete") && bootSource.includes("await globalThis.Win11SessionManager.handleBootComplete()"), "Session manager starts during boot");
check(bootSource.indexOf("await globalThis.Win11SessionManager.handleBootComplete()") < bootSource.indexOf('document.getElementById("boot")?.classList.add("hidden")'), "Boot waits for session preparation before hiding");
...
check(readFileSync(resolve(root, "service-worker.js"), "utf8").includes('win11-simulator-v10.2.0'), "PWA cache bumped to V10.2.0");
...
check(readFileSync(resolve(root, "src/features/real-platform-v660.js"), "utf8").includes('service-worker.js?v=10.2.0'), "PWA registration references V10.2 Service Worker");
...
const bootRecoveryV101=readFileSync(resolve(root,"src/core/boot-recovery-v101.js"),"utf8");
const desktopTaskbarV102=readFileSync(resolve(root,"src/core/desktop-taskbar-v102.js"),"utf8");
const bootV102=readFileSync(resolve(root,"src/core/boot.js"),"utf8");
const packageV102=JSON.parse(readFileSync(resolve(root,"package.json"),"utf8"));
const serviceWorkerV102=readFileSync(resolve(root,"service-worker.js"),"utf8");
const browserAuditV102=readFileSync(resolve(root,"tools/browser_audit.mjs"),"utf8");
...
check(index.includes("./styles/boot-recovery-v101.css?v=10.1.0"),"Boot Recovery V10.1 styles loaded");
check(index.includes("./src/core/boot-recovery-v101.js?v=10.1.0"),"Boot Recovery V10.1 module loaded");
check(bootRecoveryV101.includes('VERSION="10.1.0"')&&bootRecoveryV101.includes("globalThis.Win11BootRecovery"),"Boot Recovery V10.1 bridge present");
check(bootRecoveryV101.includes("BOOT_PHASE_TIMEOUT")&&bootRecoveryV101.includes("consecutiveIncomplete>=2"),"Boot Recovery timeout and boot-loop guards present");
check(index.includes("./src/core/desktop-taskbar-v102.js?v=10.2.0"),"Desktop / Taskbar V10.2 module loaded");
check(index.indexOf("safe-mode-v995.js?v=9.9.7")<index.indexOf("boot-recovery-v101.js?v=10.1.0")&&index.indexOf("boot-recovery-v101.js?v=10.1.0")<index.indexOf("desktop-taskbar-v102.js?v=10.2.0")&&index.indexOf("desktop-taskbar-v102.js?v=10.2.0")<index.indexOf("core/boot.js?v=10.2.0"),"V10.1 recovery and V10.2 shell load order is safe");
check(desktopTaskbarV102.includes('VERSION="10.2.0"')&&desktopTaskbarV102.includes("globalThis.Win11DesktopTaskbar"),"Desktop / Taskbar V10.2 bridge present");
check(desktopTaskbarV102.includes("pruneTaskButtons")&&desktopTaskbarV102.includes("repairTaskButtons"),"Desktop / Taskbar V10.2 self-healing present");
check(desktopTaskbarV102.includes("shell-self-reconcile")&&desktopTaskbarV102.includes("shell-integrity-health"),"Desktop / Taskbar V10.2 capability markers present");
check(bootV102.includes('VERSION="10.2.0"')&&bootV102.includes('platform.start("desktop-taskbar")'),"Boot V10.2 starts native shell surface");
check(bootV102.includes('registerCompat("desktop-integration"')&&bootV102.includes('registerCompat("taskbar-system"'),"Boot V10.2 registers shell compatibility modules");
check(bootV102.includes("desktopTaskbar:globalThis.Win11DesktopTaskbar"),"Simulator diagnostics include Desktop / Taskbar V10.2");
check(bootV102.includes('runPhase("session"')&&bootV102.includes('runPhase("recovery"')&&bootV102.includes('runPhase("shell"'),"Boot V10.2 preserves V10.1 recovery phases");
check(index.includes("./src/core/boot.js?v=10.2.0"),"Boot V10.2 module loaded");
check(packageV102.version==="10.2.0","Package version advanced to V10.2.0");
check(packageV102.scripts?.test?.includes("boot-recovery-v101.mjs")&&packageV102.scripts?.test?.includes("desktop-taskbar-v102.mjs"),"V10.1 and V10.2 unit tests wired into npm test");
check(serviceWorkerV102.includes('win11-simulator-v10.2.0'),"Service Worker cache is V10.2.0");
check(serviceWorkerV102.includes("./src/core/desktop-taskbar-v102.js?v=10.2.0")&&serviceWorkerV102.includes("./src/core/boot-recovery-v101.js?v=10.1.0"),"V10.2 shell and V10.1 recovery assets precached");
check(browserAuditV102.includes("Desktop / Taskbar V10.2")&&browserAuditV102.includes('win11-simulator-v10.2.0'),"Browser audit covers V10.2 shell and cache");
check(readFileSync(resolve(root,"README.md"),"utf8").includes("**V10.2 Desktop / Taskbar**"),"README advertises V10.2 Desktop / Taskbar");
