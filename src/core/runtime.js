...
const APPS={explorer:{name:"Explorador",icon:"📁",w:800,h:540},notepad:{name:"Notas",icon:"📝",w:720,h:500},calc:{name:"Calculadora",icon:"🧮",w:360,h:520},terminal:{name:"Terminal",icon:"⌨️",w:720,h:460},settings:{name:"Definições",icon:"⚙️",w:780,h:540},taskmanager:{name:"Gestor de Tarefas",icon:"📊",w:760,h:470},recycle:{name:"Reciclagem",icon:"🗑️",w:690,h:470},photos:{name:"Fotografias",icon:"🖼️",w:700,h:500},camera:{name:"Câmara",icon:"📷",w:760,h:560},paint:{name:"Pintar",icon:"🖌️",w:760,h:540},edge:{name:"Microsoft Edge",icon:"🌐",w:900,h:600},
...
function desktopIconSvg(kind){
  const common='viewBox="0 0 48 48" focusable="false" aria-hidden="true"';
  const icons={
    thispc:'<svg '+common+'><rect x="5" y="7" width="38" height="27" rx="3" fill="#2b78d0"/><rect x="8" y="10" width="32" height="21" rx="1.5" fill="#bfe7ff"/><path d="M17 39h14M21 34v5M27 34v5" stroke="#d8e7f2" stroke-width="3" stroke-linecap="round"/><path d="M11 15h14" stroke="#fff" stroke-width="2" opacity=".7"/></svg>',
    folder:'<svg '+common+'><path d="M5 15a4 4 0 0 1 4-4h11l4 5h15a4 4 0 0 1 4 4v17a4 4 0 0 1-4 4H9a4 4 0 0 1-4-4Z" fill="#f2b84b"/><path d="M5 20h38v17a4 4 0 0 1-4 4H9a4 4 0 0 1-4-4Z" fill="#ffd66b"/><path d="M8 23h32" stroke="#fff" stroke-width="2" opacity=".4"/></svg>',
    edge:'<svg '+common+'><defs><linearGradient id="edgeG" x1="7" y1="8" x2="40" y2="40"><stop stop-color="#0ac6d8"/><stop offset=".52" stop-color="#1786e8"/><stop offset="1" stop-color="#0a4cb4"/></linearGradient></defs><circle cx="24" cy="24" r="19" fill="url(#edgeG)"/><path d="M37 27c-2.2 7-10 12-17.2 9.4-6.8-2.4-9.4-10.2-5.8-16.1 3.2-5.3 10.4-7.6 16-4.4-8.8-.4-13 5.2-12.2 9.4.7 3.8 4.6 6.7 9.2 5.7 2.5-.5 4.6-1.8 6.3-4Z" fill="#fff" opacity=".95"/><path d="M15.5 23.5c3.2-9.1 15.7-10.6 21.1-2.2-5.2-2.4-12.8-1.8-17.5 2.2Z" fill="#65e8cf"/></svg>',
    recycle:'<svg '+common+'><path d="M15 13h18l-2 28H17Z" fill="#e9f4fb" stroke="#9cc7df" stroke-width="2"/><path d="M12 13h24M19 9h10" stroke="#d9eef9" stroke-width="3" stroke-linecap="round"/><path d="M20 20v14M28 20v14" stroke="#55a7cf" stroke-width="2"/><path d="M19 27h10" stroke="#55a7cf" stroke-width="2"/></svg>',
    settings:'<svg '+common+'><circle cx="24" cy="24" r="9" fill="#e9f4fb"/><path d="M24 5l3 4 5-1 2 5 5 2-1 5 4 4-4 4 1 5-5 2-2 5-5-1-3 4-3-4-5 1-2-5-5-2 1-5-4-4 4-4-1-5 5-2 2-5 5 1Z" fill="#4f9fda"/><circle cx="24" cy="24" r="7" fill="#eef7fc"/><circle cx="24" cy="24" r="3" fill="#4f9fda"/></svg>',
    text:'<svg '+common+'><path d="M10 5h20l8 8v30H10Z" fill="#fff" stroke="#b9c7d2" stroke-width="2"/><path d="M30 5v9h8" fill="#dcecf6"/><path d="M16 21h16M16 27h16M16 33h12" stroke="#4b8fca" stroke-width="2" stroke-linecap="round"/></svg>',
    image:'<svg '+common+'><rect x="6" y="7" width="36" height="34" rx="4" fill="#e9f4fb" stroke="#8fbfdc" stroke-width="2"/><circle cx="16" cy="17" r="4" fill="#f3c65c"/><path d="M9 36l10-10 7 7 5-5 8 8Z" fill="#4faf73"/></svg>',
    media:'<svg '+common+'><circle cx="24" cy="24" r="19" fill="#6a74da"/><path d="M20 15l14 9-14 9Z" fill="#fff"/><path d="M12 11a19 19 0 0 1 5-3" stroke="#9ee5ff" stroke-width="3" stroke-linecap="round"/></svg>'
  };
  return icons[kind]||icons.text;
}
globalThis.desktopIconSvg=desktopIconSvg;
...
function populateDesktop(){const d=$("#desktop-icons");d.innerHTML="";[["explorer","Este PC","thispc"],["explorer","Documentos","folder"],["recycle","Reciclagem","recycle"],["settings","Definições","settings"]].forEach(([app,label,iconKind],i)=>{const b=document.createElement("button");b.className="desktop-icon";b.innerHTML=`<span class="icon desktop-icon-art">${desktopIconSvg(iconKind)}</span><span class="label">${label}</span>`;b.addEventListener("dblclick",()=>openApp(app,i===1?"C:/Documents":undefined));let last=0;b.addEventListener("click",()=>{$$(".desktop-icon").forEach(x=>x.classList.remove("selected"));b.classList.add("selected");const now=Date.now();if(now-last<420)openApp(app,i===1?"C:/Documents":undefined);last=now});d.appendChild(b)})}
...
function createTaskButton(win){const a=APPS[win.dataset.app],b=document.createElement('button');b.className='task-btn running';b.dataset.window=win.dataset.id;b.textContent=a.icon;b.title=a.name;b.addEventListener('click',()=>{if(win.classList.contains('hidden')){win.classList.remove('hidden');focusWindow(win)}else if(win.classList.contains('focused'))minimizeWindow(win);else focusWindow(win)});$('#task-center').appendChild(b);updateTaskbar()}function updateTaskbar(){
  $$(".task-btn[data-window]").forEach(btn=>{
    const w=document.querySelector(`#window-layer > .window[data-id="${btn.dataset.window}"]`);
