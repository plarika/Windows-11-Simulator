...
await check("Real functions Versions V9.6 marker",async()=>await evaluate(`Win11RealFunctions?.step===29 && Win11RealFunctions.features.includes("explorer-multi-window") && Win11RealFunctions.features.includes("explorer-undo") && Win11RealFunctions.features.includes("recycle-bin-pro") && Win11RealFunctions.features.includes("explorer-previous-versions") && Win11RealFunctions.features.includes("explorer-version-restore") && Win11RealFunctions.features.includes("notepad-version-capture")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.6.0"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
