...
await check("Real functions Storage V9.8.6 marker",async()=>await evaluate(`Win11RealFunctions?.step===36 && Win11RealFunctions.features.includes("app-registry") && Win11RealFunctions.features.includes("explorer-settings-store") && Win11RealFunctions.features.includes("storage-2") && Win11RealFunctions.features.includes("storage-category-scan") && Win11RealFunctions.features.includes("storage-temporary-cleanup") && Win11RealFunctions.features.includes("storage-recycle-cleanup") && Win11RealFunctions.features.includes("storage-sense-live") && Win11RealFunctions.features.includes("storage-profile-isolation")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.8.6"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
