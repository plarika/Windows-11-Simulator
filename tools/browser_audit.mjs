...
await check("Real functions Search V9.2 marker",async()=>await evaluate(`Win11RealFunctions?.step===25 && Win11RealFunctions.features.includes("windows-experience-v8") && Win11RealFunctions.features.includes("start-menu-v3") && Win11RealFunctions.features.includes("search-v3") && Win11RealFunctions.features.includes("taskbar-jump-lists") && Win11RealFunctions.features.includes("search-query-filters") && Win11RealFunctions.features.includes("search-index-cache")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.2.0"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
