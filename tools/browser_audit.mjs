...
await check("Real functions Settings Core V9.8.1 marker",async()=>await evaluate(`Win11RealFunctions?.step===31 && Win11RealFunctions.features.includes("explorer-multi-window") && Win11RealFunctions.features.includes("explorer-undo") && Win11RealFunctions.features.includes("recycle-bin-pro") && Win11RealFunctions.features.includes("explorer-previous-versions") && Win11RealFunctions.features.includes("taskbar-app-groups") && Win11RealFunctions.features.includes("system-integration-bus") && Win11RealFunctions.features.includes("settings-profile-store") && Win11RealFunctions.features.includes("settings-atomic-commit")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.8.1"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
