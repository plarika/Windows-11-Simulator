...
await check("Real functions App Sessions V9.9.1 marker",async()=>await evaluate(`Win11RealFunctions?.step===39 && Win11RealFunctions.features.includes("shell-intent-router") && Win11RealFunctions.features.includes("app-lifecycle-events") && Win11RealFunctions.features.includes("app-session-manager") && Win11RealFunctions.features.includes("single-instance-policies") && Win11RealFunctions.features.includes("multi-instance-policies") && Win11RealFunctions.features.includes("session-activation") && Win11RealFunctions.features.includes("session-settings-ui") && Win11RealFunctions.features.includes("bounded-session-history")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.9.1"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
