...
await check("Explorer Navigation V8.2 bridge",async()=>await evaluate(`Win11ExplorerNavigation?.version==="8.2.0"`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v8.2.0"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
