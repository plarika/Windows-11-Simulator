...
await check("Real functions Apps Defaults V9.8.5 marker",async()=>await evaluate(`Win11RealFunctions?.step===35 && Win11RealFunctions.features.includes("explorer-settings-store") && Win11RealFunctions.features.includes("taskbar-system-integration") && Win11RealFunctions.features.includes("app-registry") && Win11RealFunctions.features.includes("default-apps-store") && Win11RealFunctions.features.includes("file-associations") && Win11RealFunctions.features.includes("protocol-registry") && Win11RealFunctions.features.includes("edge-local-html-safe-preview") && Win11RealFunctions.features.includes("edge-local-pdf-viewer")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.8.5"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
