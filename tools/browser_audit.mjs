...
await check("Real functions Shell Intents V9.9.0 marker",async()=>await evaluate(`Win11RealFunctions?.step===38 && Win11RealFunctions.features.includes("system-health") && Win11RealFunctions.features.includes("storage-2") && Win11RealFunctions.features.includes("shell-intent-router") && Win11RealFunctions.features.includes("ms-settings-deep-links") && Win11RealFunctions.features.includes("shell-folder-intents") && Win11RealFunctions.features.includes("virtual-path-intents") && Win11RealFunctions.features.includes("app-lifecycle-events") && Win11RealFunctions.features.includes("bounded-lifecycle-history") && Win11RealFunctions.features.includes("run-intent-routing") && Win11RealFunctions.features.includes("terminal-intent-routing") && Win11RealFunctions.features.includes("powershell-intent-routing")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.9.0"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
