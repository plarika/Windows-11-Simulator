...
await check("Real functions Taskbar System V9.8.3 marker",async()=>await evaluate(`Win11RealFunctions?.step===33 && Win11RealFunctions.features.includes("explorer-multi-window") && Win11RealFunctions.features.includes("taskbar-app-groups") && Win11RealFunctions.features.includes("system-integration-bus") && Win11RealFunctions.features.includes("settings-profile-store") && Win11RealFunctions.features.includes("settings-personalization-store") && Win11RealFunctions.features.includes("taskbar-system-integration") && Win11RealFunctions.features.includes("taskbar-auto-hide") && Win11RealFunctions.features.includes("taskbar-show-desktop") && Win11RealFunctions.features.includes("taskbar-clock-seconds")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.8.3"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
