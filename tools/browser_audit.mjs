...
await check("Run consumes HTTPS protocol registry",async()=>await evaluate(`[...document.querySelectorAll('.window[data-app="edge"] .edge-v730')].some(e=>e.__edgeV730?.getCurrent()?.url==="https://example.org/v985")`));
...
await check("Terminal start consumes protocol registry",async()=>await evaluate(`__v985TerminalResult.includes("URL aberto com a aplicação predefinida")&&[...document.querySelectorAll('.window[data-app="edge"] .edge-v730')].some(e=>e.__edgeV730?.getCurrent()?.url==="https://example.net/v985")`));
...
await check("Real functions System Health V9.8.7 marker",async()=>await evaluate(`Win11RealFunctions?.step===37 && Win11RealFunctions.features.includes("storage-2") && Win11RealFunctions.features.includes("app-registry") && Win11RealFunctions.features.includes("system-health") && Win11RealFunctions.features.includes("integration-diagnostics") && Win11RealFunctions.features.includes("settings-legacy-reconcile") && Win11RealFunctions.features.includes("integration-safe-repair") && Win11RealFunctions.features.includes("bounded-health-history") && Win11RealFunctions.features.includes("diagnostics-export") && Win11RealFunctions.features.includes("v98-integration-hardening")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.8.7"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
