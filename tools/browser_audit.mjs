...
await check("Real functions Taskbar V9.7 marker",async()=>await evaluate(`Win11RealFunctions?.step===30 && Win11RealFunctions.features.includes("explorer-multi-window") && Win11RealFunctions.features.includes("explorer-undo") && Win11RealFunctions.features.includes("recycle-bin-pro") && Win11RealFunctions.features.includes("explorer-previous-versions") && Win11RealFunctions.features.includes("taskbar-app-groups") && Win11RealFunctions.features.includes("taskbar-operation-progress") && Win11RealFunctions.features.includes("window-placement-persistence")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.7.0"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
