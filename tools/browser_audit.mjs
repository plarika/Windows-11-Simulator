...
await check("Real functions History V9.4 marker",async()=>await evaluate(`Win11RealFunctions?.step===27 && Win11RealFunctions.features.includes("windows-experience-v8") && Win11RealFunctions.features.includes("search-query-filters") && Win11RealFunctions.features.includes("explorer-multi-window") && Win11RealFunctions.features.includes("explorer-undo") && Win11RealFunctions.features.includes("explorer-redo") && Win11RealFunctions.features.includes("explorer-operation-history")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.4.0"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
