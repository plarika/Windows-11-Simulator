...
await check("Real functions Recycle V9.5 marker",async()=>await evaluate(`Win11RealFunctions?.step===28 && Win11RealFunctions.features.includes("windows-experience-v8") && Win11RealFunctions.features.includes("explorer-multi-window") && Win11RealFunctions.features.includes("explorer-undo") && Win11RealFunctions.features.includes("recycle-bin-pro") && Win11RealFunctions.features.includes("recycle-restore-all") && Win11RealFunctions.features.includes("recycle-conflict-replace")`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.5.0"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
