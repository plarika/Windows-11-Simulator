...
await check("Explorer Navigation V8.2 bridge",async()=>await evaluate(`Win11ExplorerNavigation?.version==="8.2.1"`));
await check("Explorer V8.2 initial tab",async()=>await evaluate(`(()=>{const w=document.querySelector('.window[data-app="explorer"]');return w?.__explorerNavigationV820?.getTabs().length===1&&w.querySelectorAll(".explorer-tab-v820").length===1})()`));
...
await check("Explorer tab session persisted in profile",async()=>await evaluate(`state.explorerNavigationV821?.lastSession?.tabs?.length===1&&state.explorerNavigationV821.lastSession.tabs[0]?.path==="C:/Pictures"&&state.explorerNavigationV821.closedTabs?.length>=1`));
...
await check("Explorer tab context menu",async()=>await evaluate(`(()=>{const m=document.querySelector("#context-menu.open");const t=m?.textContent||"";return t.includes("Duplicar separador")&&t.includes("Fechar outros separadores")&&t.includes("Fechar separadores à direita")})()`));
...
await check("Explorer close other tabs action",async()=>await evaluate(`document.querySelector('.window[data-app="explorer"]')?.__explorerNavigationV820?.getTabs().length===1`));
await evaluate(`(()=>{
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v8.2.1"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
