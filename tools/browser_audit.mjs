...
await check("Explorer Pro bridge",async()=>await evaluate(`Win11ExplorerPro?.version==="9.0.0"`));
...
await check("Explorer V8.9 group by date",async()=>await evaluate(`(()=>{const w=document.querySelector('.window[data-app="explorer"]'),heads=[...w.querySelectorAll(".explorer-group-heading-v890")].map(x=>x.textContent);return heads.some(x=>x.startsWith("Hoje"))&&heads.some(x=>x.startsWith("Mais antigos"))&&heads.some(x=>x.startsWith("Sem data"))})()`));
...
await check("PWA cache populated",async()=>await evaluate(`(async()=>{for(let i=0;i<25;i++){const keys=await caches.keys();if(keys.includes("win11-simulator-v9.0.0"))return true;await new Promise(x=>setTimeout(x,100))}return false})()`));
